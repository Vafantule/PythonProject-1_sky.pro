...
def flatten_values(data: Any) -> list[str]:
    """
    Рекурсивно собирает все значения из словаря и вложенных структур в список строк.
    """
    values = []
...
def count_transactions_by_category(transactions: List[Dict[str, Any]], categories: List[str]) -> Dict[str, int]:
    ...
    for transaction in transactions:
        values = [value.lower() for value in flatten_values(transaction)]
        for category, category_lower in zip(categories, categories_lower):
            if category_lower in values:
                counter[category] += 1
...
#         'to': 'American Express 0522499169905654', 'description': 'Перевод с карты на карту'},
#     ]
#     categories = ['Yuan Renminbi', 'Перевод с карты на карту']
#
